...
function updatePoliceStage4A(dt, player, cars, npcs) {
    ...
    if (player.wanted && !activePoliceUnit) {
        ...
        if (closestUnit) {
            ...
            if (isCar) {
                // Police car stops & plays existing WAIL siren
    ...
    if (activePoliceUnit) {
        ...
        if (activePoliceUnit.policeState === "WARNING") {
            ...
            // Keep unit stopped during warning
            activePoliceUnit.speed = 0;
        ...
        // ---------------------------------------------------------------------
            ...
            // Find existing officer entity (do NOT spawn duplicate officers)
            let officerNPC = null;
            if (!isCar) {
        ...
        // STATE: CHASE (Established for Stage 4B)
        
        else if (activePoliceUnit.policeState === "CHASE") {
            // Stage 4A establishes CHASE state, player.beingChased = true, and YELP siren.
            // Complex pursuit AI, path tracking, and escaping are reserved for Stage 4B.
        }
    }
}
