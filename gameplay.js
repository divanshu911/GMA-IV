...
// Existing arrest sequence logic
function executeArrestProcess() {
    taxiManager.setMessage("BUSTED: You surrendered to the police! Fine: $200", 240);
    ...
    // Remove stolen status from cars
    cars.forEach(c => { c.isStolen = false; });
    localStorage.removeItem("stolen_cars");

    // Clear player car if driving
    if (playerCar) {
        if (playerCar.humAudio) { playerCar.humAudio.pause(); playerCar.humAudio = null; }
        if (playerCar.sirenAudio) { playerCar.sirenAudio.pause(); playerCar.sirenAudio = null; }
...
function updatePoliceStage4A(dt, player, cars, npcs) {
    ...
    // Search police cars
    for (let car of cars) {
        if (car.isPolice && car.policeState && car.policeState !== "PATROL") {
    ...
    if (player.wanted && !activePoliceUnit) {
        ...
        let minDistance = 85; // Recognition range
        ...
        if (closestUnit) {
            ...
            activePoliceUnit.warningTimer = 480; // ~8 seconds (60 frames/s * 8)
            activePoliceUnit.initialDist = minDistance;

            const isCar = activePoliceUnit.length !== undefined; // Car vs NPC check
    ...
    if (activePoliceUnit) {
        ...
        if (activePoliceUnit.policeState === "WARNING") {
            // Ensure surrender button displays when warning is active
            if (surrenderBtn) surrenderBtn.style.display = 'block';

            activePoliceUnit.warningTimer -= dt;
            activePoliceUnit.speed = 0; // Keep unit stopped during warning

            const isClose = distToPlayer < 120;
            const isStopped = currentSpeed < 0.3; // Give a small grace threshold for speed

            // Add a grace period (e.g., first 1 second / 60 frames) so player isn't immediately chased
            ...
            // COMPLIED: Player stopped close enough during warning period
            if (isClose && isStopped) {
                activePoliceUnit.policeState = "ARRESTING";
                activePoliceUnit.arrestStage = 0;
                activePoliceUnit.arrestTimer = 30; // Brief delay before dialogue
                if (surrenderBtn) surrenderBtn.style.display = 'none';
            } 
            // REFUSED: Only trigger chase after grace period expires AND if player accelerates away (speed > 2.5) or runs far (dist > 250)
        ...
        // ---------------------------------------------------------------------
            ...
            // Find existing officer entity
            let officerNPC = null;
            if (!isCar) {
                officerNPC = activePoliceUnit;
            } else {
                officerNPC = activePoliceUnit.associatedOfficer || 
                             npcs.find(n => n.isPolice && Math.hypot(n.x - unitX, n.y - unitY) < 160) || 
                             null;
            }

            // --- STEP A: Player inside vehicle -> "Step out!" ---
            if (playerCar && activePoliceUnit.arrestStage === 0) {
                ...
                // Force exit using existing vehicle exit mechanics
                ...
                activePoliceUnit.arrestTimer = 60; // Delay for "You are arrested!"
                return;
            }

            // --- STEP B: Player on foot -> "You are arrested!" ---
            if (officerNPC && typeof officerNPC.say === 'function') {
                officerNPC.say("You are arrested!", 120);
            }

            // Execute Arrest Results
            // 1. Remove all stolen vehicles from world array
            ...
            // 2. Clear wanted and chasing states
            player.wanted = false;
            player.beingChased = false;

            // 3. Sync state to localStorage using existing helper
            if (typeof updateStolenCarsStorage === 'function') {
                updateStolenCarsStorage();
            }

            // 4. Return police unit to normal behavior
            if (isCar) {
                if (typeof activePoliceUnit.stopSiren === 'function') {
                    activePoliceUnit.stopSiren();
                } else {
        ...
        // ---------------------------------------------------------------------
        // STATE: CHASE (Without actual chase pursuit movement)
        // ---------------------------------------------------------------------
            ...
            // Pursuit movement intentionally omitted per specification.
        }
    } else {
        if (surrenderBtn) surrenderBtn.style.display = 'none';
    }
