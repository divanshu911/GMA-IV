  ...
  // --- STATE: CHASE ---
    ...
    if (distToPlayer > 30) {
        ...
        if (isCar) {
            ...
            const chaseSpeed = 1.6; // Slightly faster pursuit speed for foot officers
            const nextX = activePoliceUnit.x + Math.cos(moveAngle) * chaseSpeed * dt;
            const nextY = activePoliceUnit.y + Math.sin(moveAngle) * chaseSpeed * dt;

            // Verify both road and grass terrain allow step
            // --- OFFICER TERRAIN MOVEMENT ---
// Use the officer's actual center position.
// Grass, road and transition are all valid for on-foot police.
...
if (nextTerrainWalkable) {
    ...
    // Try sliding along one axis when crossing an edge.
    const xOnlyWalkable = isGrassOrRoad(nextX, activePoliceUnit.y);
    const yOnlyWalkable = isGrassOrRoad(activePoliceUnit.x, nextY);

    if (xOnlyWalkable && yOnlyWalkable) {
        // Pick whichever gets closer to the player.
        ...
        if (xDist <= yDist) {
            ...
            activePoliceUnit.y = nextY;
        } activePoliceUnit.walkTimer = (activePoliceUnit.walkTimer || 0) + chaseSpeed * dt * 0.12;
