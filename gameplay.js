...
function updateSinglePoliceChase(unit, dt, player, cars, npcs) {
    ...
    if (distToPlayer > 30) {
        ...
        cars.forEach(otherCar => {
            if (otherCar !== unit && otherCar !== playerCar) {
                const d = Math.hypot(otherCar.x - unit.x, otherCar.y - unit.y);
                if (d < avoidanceRadius && d > 0) {
                    avoidX -= (otherCar.x - unit.x) / d;
                    avoidY -= (otherCar.y - unit.y) / d;
                }
            }
        });
        ...
        npcs.forEach(npc => {
            if (npc !== unit) {
                const d = Math.hypot(npc.x - unit.x, npc.y - unit.y);
                if (d < avoidanceRadius && d > 0) {
                    avoidX -= (npc.x - unit.x) / d;
                    avoidY -= (npc.y - unit.y) / d;
                }
            }
        });
