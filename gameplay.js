...
surrenderBtn.addEventListener('click', () => {
  ...
  // --- STATE: CHASE ---
    ...
    if (isCar) {
        activePoliceUnit.isParked = false; // FIX 2: Ensure movement system updates position
        if (typeof activePoliceUnit.playSiren === 'function') {
    ...
    if (distToPlayer > 30) {
        // Query the existing A* Navigation System
        const path = navigationSystem.findPath(activePoliceUnit.x, activePoliceUnit.y, player.x, player.y);

        if (path && path.length > 1) {
            // Target the next immediate waypoint in the generated path
            const nextWaypoint = path[1];
            let targetAngle = Math.atan2(nextWaypoint.y - activePoliceUnit.y, nextWaypoint.x - activePoliceUnit.x) + Math.PI / 2;
            ...
            // Blend A* trajectory with the avoidance steering vector if obstacles exist
            if (avoidX !== 0 || avoidY !== 0) {
                const avoidAngle = Math.atan2(avoidY, avoidX) + Math.PI / 2;
                targetAngle = targetAngle * 0.6 + avoidAngle * 0.4; // Soft interpolate
            }

            // Apply movement updates
            activePoliceUnit.angle = targetAngle;
            activePoliceUnit.speed = isCar ? (activePoliceUnit.baseSpeed || 1.8) : 1.2;

        } else {
            // Fallback: Use direct line of sight if A* fails to find a valid grid path
            activePoliceUnit.angle = Math.atan2(player.y - activePoliceUnit.y, player.x - activePoliceUnit.x) + Math.PI / 2;
            activePoliceUnit.speed = isCar ? (activePoliceUnit.baseSpeed || 1.8) : 1.2;
        }
    } else {
