  ...
  // --- STATE: CHASE ---
    ...
    if (distToPlayer > 30) {
        ...
        if (isCar) {
            ...
            if (typeof isWalkableColor === 'function' ? isWalkableColor(nextX, nextY, activePoliceUnit.size || 20) : isGrassOrRoad(nextX, nextY)) {
                activePoliceUnit.x = nextX;
                activePoliceUnit.y = nextY;
            } else {
                // If direct step hits an edge, attempt single-axis sliding along walkable terrain
                if (isGrassOrRoad(nextX, activePoliceUnit.y)) {
                    activePoliceUnit.x = nextX;
                } else if (isGrassOrRoad(activePoliceUnit.x, nextY)) {
                    activePoliceUnit.y = nextY;
                }
            }
