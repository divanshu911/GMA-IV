...
if (blackMarketBtn) {
    ...
    blackMarketBtn.addEventListener('click', () => {
        ...
        // Remove sold car from stolen history if present
        if (typeof stolenCarIds !== 'undefined') {
            stolenCarIds = stolenCarIds.filter(id => id !== carToSell.id);
            localStorage.setItem("gma_stolen_car_ids", JSON.stringify(stolenCarIds));

            // Reset wanted state if all stolen cars have been sold or abandoned
            if (stolenCarIds.length === 0 && player.wanted) {
                clearWantedState();
                taxiManager.setMessage(`Sold ${carType}! Police dropped all charges.`, 180);
            }
        }
...
function getNearbyRoadPosition(startX, startY) {
    const minRadius = 15; // Minimum 15 pixels away from current position
    const maxRadius = 1000;
    const step = 15;

    // Search outward in expanding concentric circles starting at minRadius (15px)
...
class PoliceSystem {
  constructor() {
    ...
    this.arrestTimer = 0;           // Timer for speech bubble transitions
    this.arrestingOfficer = null;   // Reference to officer conducting arrest
    this.spawnCarTimer = 0;         // Timer for spawning off-screen police during chase
    this.spawnInterval = 600;       // ~10 seconds (600 frames) between extra spawns
    this.movementHistory = [];      // Player trail for path-following pursuit AI
    this.historyTimer = 0;
  }

  // Record player position history for AI path tracking
  recordPlayerHistory(player) {
    this.historyTimer++;
    if (this.historyTimer % 10 === 0) { // Record every 10 frames
      this.movementHistory.push({ x: player.x, y: player.y });
      if (this.movementHistory.length > 25) {
        this.movementHistory.shift(); // Maintain recent trail
      }
    }
  }
  ...
  update(dt, player, cars, npcs) {
    ...
    // Ensure player has beingChased property initialized
    if (player.beingChased === undefined) player.beingChased = false;

    // Track player movement trail when wanted or actively chased
    if (player.wanted || player.beingChased) {
      this.recordPlayerHistory(player);
    } else {
      this.movementHistory = [];
      player.beingChased = false;
      this.isSpotted = false;
      this.activePoliceUnit = null;
      return;
    }

    // --- 1. DETECT NEARBY POLICE UNITS ---
    let nearestPolice = this.findNearestPoliceUnit(player, cars, npcs);

    // Escape Condition: If player moves sufficiently far (> 900px) from all police units during a chase
    if (player.beingChased) {
      if (!nearestPolice || nearestPolice.dist > 900) {
        this.escapeChase(player, cars);
        return;
      }
    }
    ...
    if (player.wanted && !player.beingChased && !this.isSpotted && nearestPolice && nearestPolice.dist < 280) {
      ...
      // Play WAIL Siren if interceptor is a police car
      if (nearestPolice.isCar && typeof nearestPolice.unit.setSirenState === 'function') {
        nearestPolice.unit.setSirenState(1); // 1 = WAIL
      }
    }

    // Processing Spotted Window (Count Down)
    if (this.isSpotted && !player.beingChased) {
      this.spottedTimer -= 1 * dt;

      // Bring intercepting car to a stop
      if (this.activePoliceUnit && this.activePoliceUnit.width !== undefined) {
        this.activePoliceUnit.speed = Math.max(0, this.activePoliceUnit.speed - 0.1 * dt);
      }
      ...
      if (isPlayerStopped && isNearPolice) {
        this.triggerArrestSequence(player, cars, npcs);
        return;
      }
    ...
    // --- 3. CHASE PHASE LOGIC ---
    if (player.beingChased) {
      // Allow player to surrender by stopping near police during chase
      let isPlayerStopped = Math.abs(player.speed || 0) < 0.2;
      if (isPlayerStopped && nearestPolice && nearestPolice.dist < 90) {
        this.triggerArrestSequence(player, cars, npcs);
        return;
      }

      // Handle Pursuit AI (Updates vehicle chase targets and NPC pathing)
      this.updatePolicePursuit(dt, player, cars, npcs);

      // Spawn off-screen police cars periodically during chase
      this.spawnCarTimer += 1 * dt;
      if (this.spawnCarTimer >= this.spawnInterval) {
        this.spawnCarTimer = 0;
        this.spawnOffScreenPoliceCar(player, cars);
      }
    }

    // --- 4. ARREST SEQUENCE HANDLING ---
    if (this.arrestState !== "NONE") {
      this.updateArrestSequence(dt, player, cars, npcs);
    }
  }

  // Locate nearest active Police Car or Officer NPC
  findNearestPoliceUnit(player, cars, npcs) {
    let closest = null;
    let minDist = Infinity;

    if (cars && cars.length) {
      cars.forEach(car => {
        if (car.isPolice && car.health > 0 && !car.exploded) {
          let d = Math.hypot(car.x - player.x, car.y - player.y);
          if (d < minDist) {
            minDist = d;
            closest = { unit: car, dist: d, isCar: true };
          }
        }
      });
    }
    ...
    if (npcs && npcs.length) {
      npcs.forEach(npc => {
        if (npc.isPolice && !npc.isPassenger) {
          let d = Math.hypot(npc.x - player.x, npc.y - player.y);
          if (d < minDist) {
            minDist = d;
            closest = { unit: npc, dist: d, isCar: false };
          }
        }
      });
    }
  ...
  startChase(player, cars) {
    ...
    // Switch all active police cars to YELP siren (2)
    cars.forEach(c => {
      if (c.isPolice && c.health > 0 && typeof c.setSirenState === 'function') {
        c.setSirenState(2); // 2 = YELP
      }
    });
  }

  escapeChase(player, cars) {
    player.beingChased = false;
    this.isSpotted = false;
    this.activePoliceUnit = null;

    if (typeof taxiManager !== 'undefined' && taxiManager.setMessage) {
      taxiManager.setMessage("You escaped the chase!", 200);
    }

    // Silence all police sirens and clear chase targets
    cars.forEach(c => {
      if (c.isPolice) {
        if (typeof c.setSirenState === 'function') c.setSirenState(0);
        c.chaseTarget = null;
      }
    });
  }

  // --- POLICE PURSUIT AI INTEGRATION ---
  updatePolicePursuit(dt, player, cars, npcs) {
    // 1. Target determination: Movement history trail or player position
    let targetX = player.x;
    let targetY = player.y;

    if (this.movementHistory.length > 5) {
      let trailPoint = this.movementHistory[Math.floor(this.movementHistory.length / 2)];
      targetX = trailPoint.x;
      targetY = trailPoint.y;
    }

    // 2. Assign chase target to police cars (integrates with Car.updateAI)
    cars.forEach(car => {
      if (car.isPolice && car.health > 0 && !car.isParked && car.hasDriver) {
        car.chaseTarget = { x: targetX, y: targetY };
      }
    });

    // 3. Officer NPCs Foot Pursuit (using standard walkable/road collision)
    npcs.forEach(npc => {
      if (npc.isPolice && !npc.isPassenger) {
        ...
        if (dist > 5) {
          ...
          npc.angle = angle + Math.PI / 2;
          let runSpeed = 2.0 * dt;

          let nextX = npc.x + Math.cos(angle) * runSpeed;
          let nextY = npc.y + Math.sin(angle) * runSpeed;

          if (typeof isWalkableColor === 'function' && isWalkableColor(nextX, nextY, npc.size)) {
            npc.x = nextX;
            npc.y = nextY;
          } else if (typeof isRoadColor === 'function' && isRoadColor(nextX, nextY)) {
            npc.x = nextX;
            npc.y = nextY;
          }
          npc.walkTimer += 0.25 * dt;
        }
      }
    });
  }

  // --- OFF-SCREEN POLICE CAR SPAWNER ---
  spawnOffScreenPoliceCar(player, cars) {
    let spawnAngle = Math.random() * Math.PI * 2;
    let distance = 750; // Outside viewport
    let spawnX = player.x + Math.cos(spawnAngle) * distance;
    let spawnY = player.y + Math.sin(spawnAngle) * distance;

    if (typeof getRandomRoadPosition === 'function') {
      let roadPos = getRandomRoadPosition();
      if (Math.hypot(roadPos.x - player.x, roadPos.y - player.y) > 500) {
        spawnX = roadPos.x;
        spawnY = roadPos.y;
      }
    }

    let policeCar = new Car(cars.length + 8500, spawnX, spawnY, "#111111", true, "Commuter, Sedan");
    policeCar.isPolice = true;
    policeCar.hasDriver = true;
    policeCar.isParked = false;
    policeCar.baseSpeed = 1.6;
    if (typeof policeCar.setSirenState === 'function') policeCar.setSirenState(2); // YELP

    cars.push(policeCar);
  }

  // --- ARREST SEQUENCE HANDLER ---
  triggerArrestSequence(player, cars, npcs) {
    this.isSpotted = false;
    this.arrestTimer = 0;

    let nearest = this.findNearestPoliceUnit(player, cars, npcs);
    let policeUnit = this.activePoliceUnit || (nearest ? nearest.unit : null);
    let officerNPC = null;

    if (policeUnit) {
      if (policeUnit.setSirenState) policeUnit.setSirenState(0); // Turn off siren

      if (policeUnit.width !== undefined) { // Interceptor is a Car
        policeUnit.isParked = true;
        policeUnit.speed = 0;
        policeUnit.chaseTarget = null;

        // Reuse existing assigned officer NPC or spawn one if car has driver and no officer assigned
        if (policeUnit.assignedOfficer) {
          officerNPC = policeUnit.assignedOfficer;
        } else {
          policeUnit.hasDriver = false;
          let sideAngle = policeUnit.angle + Math.PI / 2;
          officerNPC = new NPC(
            Date.now(),
            policeUnit.x + Math.cos(sideAngle) * 28,
            policeUnit.y + Math.sin(sideAngle) * 28,
            "#0b1d3a", "#2d3436", "#ffdbac",
            true // isPolice = true
          );
          officerNPC.assignedCar = policeUnit;
          policeUnit.assignedOfficer = officerNPC;
          npcs.push(officerNPC);
        }
      } else {
        // Interceptor was already an Officer NPC on foot
        officerNPC = policeUnit;
      }
    }

    this.arrestingOfficer = officerNPC;

    // Check if player is currently driving a vehicle
    if (typeof playerCar !== 'undefined' && playerCar) {
      this.arrestState = "STEP_OUT";
      if (this.arrestingOfficer) {
        this.arrestingOfficer.say("Step out!", 120);
      }
    } else {
      this.arrestState = "ARREST_SPEECH";
      if (this.arrestingOfficer) {
        this.arrestingOfficer.say("You are arrested!", 150);
      }
    }
  }

  updateArrestSequence(dt, player, cars, npcs) {
    this.arrestTimer += 1 * dt;

    if (this.arrestState === "STEP_OUT") {
      // Check if player has stepped out of vehicle or force exit after short delay (~1.5s)
      if (typeof playerCar === 'undefined' || !playerCar || this.arrestTimer > 90) {
        if (typeof playerCar !== 'undefined' && playerCar) {
    ...
    if (this.arrestState === "ARREST_SPEECH") {
      if (this.arrestTimer > 120) {
        this.finalizeArrest(player, cars, npcs);
      }
    }
  }

  finalizeArrest(player, cars, npcs) {
    // 1. Remove stolen vehicle from world & local storage
    let stolenCarIndex = cars.findIndex(c => c.isStolen);
    if (stolenCarIndex > -1) {
      cars.splice(stolenCarIndex, 1);
    }
    localStorage.removeItem("stolen car");

    // 2. reset wanted/chased status
    player.wanted = false;
    player.beingChased = false;
    ...
    if (typeof taxiManager !== 'undefined' && taxiManager.setMessage) {
      taxiManager.setMessage("Arrested! Paid $150 bail fee.", 240);
    }

    // 3. Handle Officer return to duty
    if (this.arrestingOfficer) {
      if (this.arrestingOfficer.assignedCar) {
        let car = this.arrestingOfficer.assignedCar;
        car.hasDriver = true;
        car.isParked = false;
        car.assignedOfficer = null;
        if (typeof car.setSirenState === 'function') car.setSirenState(0);

        // Remove the foot officer NPC entity as officer returns to driving
        let idx = npcs.indexOf(this.arrestingOfficer);
        if (idx > -1) npcs.splice(idx, 1);
      } else {
        // Officer on foot resumes normal patrol
        this.arrestingOfficer.speechText = null;
      }
    }

    // Reset Police Manager internal states
    this.arrestState = "NONE";
    this.arrestTimer = 0;
    this.activePoliceUnit = null;
