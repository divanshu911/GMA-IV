...
function updatePoliceStage4A(dt, player, cars, npcs) {
    ...
    if (activePoliceUnit) {
        ...
        if (activePoliceUnit.policeState === "WARNING") {
            ...
            const isStopped = currentSpeed < 0.2;
            ...
            // REFUSED: Player drives/runs away or warning timer expires
            else if ((distToPlayer > activePoliceUnit.initialDist + 80 || currentSpeed > 1.2) || activePoliceUnit.warningTimer <= 0) {
                player.beingChased = true;
                activePoliceUnit.policeState = "CHASE";
                if (surrenderBtn) surrenderBtn.style.display = 'none';

                if (isCar) {
                    // Switch to existing YELP siren
                    if (typeof activePoliceUnit.playSiren === 'function') {
