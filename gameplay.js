...
function executeArrestProcess() {
    ...
    // Clear stolen car storage and wanted state
...
localStorage.setItem("gma_player_wanted", "false");
    ...
    // IF the player was driving a stolen car, remove it from the active world array entirely
    if (playerCar.isStolen) {
        cars = cars.filter(c => c.id !== playerCar.id);
    } else {
...
// Remove ALL stolen cars from the active world array upon arrest
cars = cars.filter(c => !c.isStolen);
...
function updatePoliceStage4A(dt, player, cars, npcs) {
    ...
    if (!player.wanted && (!activePoliceUnit || activePoliceUnit.policeState !== "ARRESTING")) {
        if (activePoliceUnit) {
            ...
            // FIX: Restore foot-officer walking speed if cleared early
    ...
    if (player.wanted && !activePoliceUnit) {
        ...
        cars.forEach(c => {
    if (c.isPolice && c !== playerCar && (!c.policeState || c.policeState === "PATROL")) {
    ...
    if (activePoliceUnit) {
        ...
        if (activePoliceUnit.policeState === "WARNING") {
            // Keep surrender button displayed while warning is active
            if (surrenderBtn) surrenderBtn.style.display = 'block';

            activePoliceUnit.warningTimer -= dt;

            // FIX: Only freeze speed if the active unit is NOT the player's current vehicle
            ...
            // SURRENDERED: Player manually pressed the surrender button
            ...
            // REFUSED / FLED: Trigger chase if player accelerates away, flees, or timer expires
    ...
    // FIX: Restore foot-officer walking speed so they resume patrolling
    activePoliceUnit.speed = 0.3 + Math.random() * 0.4; 
            }
