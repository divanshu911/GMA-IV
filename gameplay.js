...
surrenderBtn.style.bottom = '80px';
...
function drawPoliceBullets(ctx) {
    ...
    ctx.restore();
                                   ...
                                   // Safety fallback for player chased property
                                   if (player.beingChased === undefined) player.beingChased = false;

                                   const surrenderBtn = document.getElementById('surrenderBtn');

                                   // Find any police unit currently interacting with the player
                                   let activePoliceUnit = null;
                                   ...
                                   for (let car of cars) {
                                       if (car.isPolice &&
                                           ...
                                           activePoliceUnit = car;
                                           break;
                                       }
                                   }

                                   // Search on-foot officer NPCs
                                   if (!activePoliceUnit) {
                                       for (let npc of npcs) {
                                           if (npc.isPolice && npc.policeState && npc.policeState !== "PATROL") {
                                               activePoliceUnit = npc;
                                               break;
                                           }
                                       }
                                   }

                                   // Reset state if player is clear of wanted status and not being arrested
                                   if (!player.wanted && (!activePoliceUnit || activePoliceUnit.policeState !== "ARRESTING")) {
                                       if (activePoliceUnit) {
                                           if (activePoliceUnit.length !== undefined) {
                                               if (typeof activePoliceUnit.stopSiren === 'function') {
                                                   activePoliceUnit.stopSiren();
                                               } else {
                                                   activePoliceUnit.sirenState = 0;
                                               }
                                               activePoliceUnit.isParked = false;
                                               activePoliceUnit.policeState = "PATROL";
                                               activePoliceUnit.warningTimer = 0;
                                               activePoliceUnit.graceTimer = 0;
                                               activePoliceUnit.arrestStage = 0;
                                               activePoliceUnit.arrestTimer = 0;
                                               activePoliceUnit.saidStepOut = false;
                                               activePoliceUnit.saidArrested = false;
                                           } else {
                                               // Restore foot-officer walking speed if cleared early
                                               activePoliceUnit.speed = 0.3 + Math.random() * 0.4;
                                               activePoliceUnit.policeState = "PATROL";
                                               activePoliceUnit.saidStepOut = false;
                                               activePoliceUnit.saidArrested = false;
                                           }
                                   ...
                                   // 1. POLICE DETECTS WANTED PLAYER
                                   if (player.wanted && !activePoliceUnit) {
                                       let closestUnit = null;
                                       let minDistance = 110; // Recognition range
   ...
   // Check nearby officer NPCs
                                       ...
                                       // Only ONE nearby unit reacts
                                       if (closestUnit) {
                                           activePoliceUnit = closestUnit;
                                           activePoliceUnit.policeState = "WARNING";
                                           activePoliceUnit.warningTimer = 480; // ~8 seconds
                                           activePoliceUnit.initialDist = minDistance;
                                           activePoliceUnit.saidStepOut = false;
                                           activePoliceUnit.saidArrested = false;

                                           const isCar = activePoliceUnit.length !== undefined;

                                           if (isCar) {
                                               // Police car stops & plays WAIL siren
                                               activePoliceUnit.speed = 0;
                                               activePoliceUnit.isParked = true;
                                               if (typeof activePoliceUnit.playSiren === 'function') {
                                                   activePoliceUnit.playSiren(1); // 1 = WAIL
                                               } else {
                                                   activePoliceUnit.sirenState = 1;
                                               }
                                           } else {
                                               // Officer NPC stops walking
                                               activePoliceUnit.speed = 0;
                                           }

                                           // Display warning using existing HUD system
                                   ...
                                   // 2. ACTIVE POLICE UNIT STATE MACHINE
                                   if (activePoliceUnit) {
                                       const isCar = activePoliceUnit.length !== undefined;
                                       const unitX = activePoliceUnit.x;
                                       const unitY = activePoliceUnit.y;

                                       const distToPlayer = Math.hypot(player.x - unitX, player.y - unitY);
                                       const currentSpeed = playerCar ? Math.abs(playerCar.speed) : Math.abs(player.speed || 0);

                                       // Retrieve nearby or active on-foot officer NPC for speech bubbles
                                       let officerNPC = !isCar ? activePoliceUnit : (
                                           activePoliceUnit.associatedOfficer || 
                                           npcs.find(n => n.isPolice && Math.hypot(n.x - unitX, n.y - unitY) < 200) || 
                                           null
                                       );

                                       // STATE: STOP WARNING
                                       if (activePoliceUnit.policeState === "WARNING") {
                                           if (surrenderBtn) surrenderBtn.style.display = 'block';

                                           activePoliceUnit.warningTimer -= dt;

                                           if (activePoliceUnit !== playerCar) {
                                               activePoliceUnit.speed = 0; 
                                           }

                                           if (!activePoliceUnit.graceTimer) activePoliceUnit.graceTimer = 60;
                                           if (activePoliceUnit.graceTimer > 0) activePoliceUnit.graceTimer -= dt;

                                           // Trigger speech bubbles on the on-foot officer NPC during WARNING stage
                                           if (officerNPC && typeof officerNPC.say === 'function') {
                                               if (playerCar) {
                                                   // Player is inside a vehicle: Say "Step out!" first
                                                   if (!activePoliceUnit.saidStepOut) {
                                                       officerNPC.say("Step out!", 140);
                                                       activePoliceUnit.saidStepOut = true;
                                                   } 
                                                   // Then say "You are arrested!" after initial warning
                                                   else if (activePoliceUnit.warningTimer <= 340 && !activePoliceUnit.saidArrested) {
                                                       officerNPC.say("You are arrested!", 180);
                                                       activePoliceUnit.saidArrested = true;
                                                   }
                                               } else {
                                                   // Player is on foot: Skip "Step out!" and say "You are arrested!" directly
                                                   if (!activePoliceUnit.saidArrested) {
                                                       officerNPC.say("You are arrested!", 180);
                                                       activePoliceUnit.saidArrested = true;
                                                   }
                                               }
                                           ...
                                           if (isPlayerSurrendered) {
                                               isPlayerSurrendered = false;
                                               activePoliceUnit.policeState = "ARRESTING";
                                               activePoliceUnit.arrestStage = 0;
                                               activePoliceUnit.arrestTimer = 30;
                                               if (surrenderBtn) surrenderBtn.style.display = 'none';
                                           } 
        // --- WARNING TRANSITION TO CHASE ---
else if (activePoliceUnit.graceTimer <= 0 && 
        (distToPlayer > 250 || currentSpeed > 2.5 || activePoliceUnit.warningTimer <= 0)) {
    player.beingChased = true;
    activePoliceUnit.policeState = "CHASE";
    if (surrenderBtn) surrenderBtn.style.display = 'none';

    if (isCar) {
        activePoliceUnit.isParked = false; // FIX 1: Unpark vehicle when chase begins
        if (typeof activePoliceUnit.playSiren === 'function') {
            activePoliceUnit.playSiren(2); // 2 = YELP
        } else {
            activePoliceUnit.sirenState = 2;
        }
    }
    ...
    if (typeof taxiManager !== 'undefined' && taxiManager.setMessage) {
        taxiManager.setMessage("You are being chased!", 180);
                                       ...
                                       // STATE: ARREST SEQUENCE
                                       else if (activePoliceUnit.policeState === "ARRESTING") {
                                           if (surrenderBtn) surrenderBtn.style.display = 'none';

                                           if (activePoliceUnit.arrestTimer > 0) {
                                               activePoliceUnit.arrestTimer -= dt;
                                               return;
                                           }

                                           if (playerCar && activePoliceUnit.arrestStage === 0) {
                                               if (officerNPC && typeof officerNPC.say === 'function') {
                                                   officerNPC.say("Step out!", 120);
                                               } else if (typeof taxiManager !== 'undefined' && taxiManager.setMessage) {
                                                   taxiManager.setMessage("Police: Step out!", 120);
                                               }
                                               ...
                                               if (typeof playSpatialSound === 'function' && typeof carStopPool !== 'undefined') {
                                               ...
                                               activePoliceUnit.arrestStage = 1;
                                               activePoliceUnit.arrestTimer = 60;
                                               return;
                                           }

                                           if (officerNPC && typeof officerNPC.say === 'function') {
                                               officerNPC.say("You are arrested!", 120);
                                           }

                                           for (let i = cars.length - 1; i >= 0; i--) {
                                               if (cars[i].isStolen) {
                                                   if (cars[i].humAudio) {
                                                       cars[i].humAudio.pause();
                                                       cars[i].humAudio = null;
                                                   }
                                                   cars.splice(i, 1);
                                               }
                                           }
                                           ...
                                           if (typeof updateStolenCarsStorage === 'function') {
                                               updateStolenCarsStorage();
                                           }
  ...
  // --- STATE: CHASE ---
else if (activePoliceUnit.policeState === "CHASE") {
    if (surrenderBtn) surrenderBtn.style.display = 'none';
    // ============================================================
// ON-FOOT OFFICER VS MOVING PLAYER CAR
// The same officer that detected the player becomes a roadside
// vehicle interceptor. No additional police unit is created.
// ============================================================
...
if (!isCar && playerCar) {
    ...
    if (playerCarSpeed > POLICE_OFFICER_MIN_CAR_SPEED) {
        // Officer stops instead of chasing the moving car.
        activePoliceUnit.speed = 0;

        const dx = playerCar.x - activePoliceUnit.x;
        const dy = playerCar.y - activePoliceUnit.y;
        const carDistance = Math.hypot(dx, dy);

        // Face the player's moving car.
        if (carDistance > 0) {
            activePoliceUnit.angle =
                Math.atan2(dy, dx) + Math.PI / 2;
        }

        // Initialize firing cooldown.
        if (activePoliceUnit.policeFireCooldown === undefined) {
            activePoliceUnit.policeFireCooldown = 0;
        }

        activePoliceUnit.policeFireCooldown -= dt;

        // Fire only when the car is within range.
        if (
            carDistance <= POLICE_OFFICER_SHOOT_RANGE &&
            activePoliceUnit.policeFireCooldown <= 0
        ) {
            fireOfficerBullet(activePoliceUnit, playerCar);
            activePoliceUnit.policeFireCooldown =
                POLICE_OFFICER_FIRE_COOLDOWN;
        }

        // Do NOT execute the normal officer chase movement below.
        return;
    }
    ...
    if (isCar) {
        activePoliceUnit.isParked = false; 
        if (typeof activePoliceUnit.playSiren === 'function') {
            activePoliceUnit.playSiren(2);
        } else {
            activePoliceUnit.sirenState = 2;
        }
    }

    // --- A* CHASE LOGIC & MOVEMENT ---
    const distToPlayer = Math.hypot(player.x - activePoliceUnit.x, player.y - activePoliceUnit.y);

    // Continue chasing only if further than 30 pixels
    if (distToPlayer > 30) {
        // Query A* Navigation System (Grid already incorporates ROAD, GRASS, and TRANSITION)
        const path = navigationSystem.findPath(activePoliceUnit.x, activePoliceUnit.y, player.x, player.y);

        let moveAngle = activePoliceUnit.angle;

        if (path && path.length > 1) {
            // Target the next immediate waypoint in the path
            const nextWaypoint = path[1];
            moveAngle = Math.atan2(nextWaypoint.y - activePoliceUnit.y, nextWaypoint.x - activePoliceUnit.x);
        } else {
            // Fallback: Direct line-of-sight direction to player
            moveAngle = Math.atan2(player.y - activePoliceUnit.y, player.x - activePoliceUnit.x);
        }

        // --- DYNAMIC AVOIDANCE (Cars and NPCs) ---
        let avoidX = 0, avoidY = 0;
        const avoidanceRadius = isCar ? 45 : 25; 

        cars.forEach(otherCar => {
            if (otherCar !== activePoliceUnit && otherCar !== playerCar) {
                const d = Math.hypot(otherCar.x - activePoliceUnit.x, otherCar.y - activePoliceUnit.y);
                if (d < avoidanceRadius && d > 0) {
                    avoidX -= (otherCar.x - activePoliceUnit.x) / d;
                    avoidY -= (otherCar.y - activePoliceUnit.y) / d;
                }
            }
        });

        npcs.forEach(npc => {
            if (npc !== activePoliceUnit) {
                const d = Math.hypot(npc.x - activePoliceUnit.x, npc.y - activePoliceUnit.y);
                if (d < avoidanceRadius && d > 0) {
                    avoidX -= (npc.x - activePoliceUnit.x) / d;
                    avoidY -= (npc.y - activePoliceUnit.y) / d;
                }
            }
        });

        // Blend angle if obstacle avoidance is active
        if (avoidX !== 0 || avoidY !== 0) {
            const avoidAngle = Math.atan2(avoidY, avoidX);
            moveAngle = moveAngle * 0.6 + avoidAngle * 0.4;
        }

        // Set visual rotation angle (converting standard radians to game canvas sprite angle)
        activePoliceUnit.angle = moveAngle + Math.PI / 2;

        // Apply Speed & Movement Updates
        if (isCar) {
    const policeChaseSpeed =
        activePoliceUnit.baseSpeed || 1.8;

    activePoliceUnit.speed = policeChaseSpeed;

    const nextX =
        activePoliceUnit.x +
        Math.cos(moveAngle) * policeChaseSpeed * dt;

    const nextY =
        activePoliceUnit.y +
        Math.sin(moveAngle) * policeChaseSpeed * dt;

    if (isGrassOrRoad(nextX, nextY)) {
        activePoliceUnit.x = nextX;
        activePoliceUnit.y = nextY;
    } else {
        const xOnlyWalkable =
            isGrassOrRoad(nextX, activePoliceUnit.y);

        const yOnlyWalkable =
            isGrassOrRoad(activePoliceUnit.x, nextY);

        if (xOnlyWalkable && yOnlyWalkable) {
            const xDist = Math.hypot(
                player.x - nextX,
                player.y - activePoliceUnit.y
            );

            const yDist = Math.hypot(
                player.x - activePoliceUnit.x,
                player.y - nextY
            );
    ...
    // On-foot Officer NPC: Apply position 
            // On-foot Officer NPC: Apply position updates and terrain movement verification
           const chaseSpeed = 1.6;
const oldX = activePoliceUnit.x;
const oldY = activePoliceUnit.y;

const nextX =
    activePoliceUnit.x + Math.cos(moveAngle) * chaseSpeed * dt;
const nextY =
    activePoliceUnit.y + Math.sin(moveAngle) * chaseSpeed * dt;

const nextTerrainWalkable = isGrassOrRoad(nextX, nextY);

if (nextTerrainWalkable) {
    activePoliceUnit.x = nextX;
    activePoliceUnit.y = nextY;
} else {
    const xOnlyWalkable =
...
// Animate only if the officer actually moved.
const movedDistance = Math.hypot(
    activePoliceUnit.x - oldX,
    activePoliceUnit.y - oldY
);

if (movedDistance > 0.001) {
    activePoliceUnit.walkTimer =
        (activePoliceUnit.walkTimer || 0) + chaseSpeed * dt * 0.12;

    activePoliceUnit.speed = chaseSpeed;
} else {
    // The officer was actually blocked, so don't let the
    // footstep system think he is walking.
    activePoliceUnit.speed = 0;
}
        }
    } else {
        // Halt completely when close enough to trigger arrest or interaction
        activePoliceUnit.speed = 0;
    }
}
  }                                   }  
