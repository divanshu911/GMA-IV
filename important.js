...
function isRoadColor(x, y) {
  ...
  // Roads use darker neutral grey. Exclude light-grey buildings and
  // bright white road markings from road samples.
  const isDarkGreyRoad = (
    Math.abs(r - g) < 35 &&
    Math.abs(g - b) < 35 &&
    Math.abs(r - b) < 35 &&
    r >= 30 &&
  ...
  const isWhiteRoadLine = r > 180 && g > 180 && b > 180;

  return isDarkGreyRoad || isWhiteRoadLine;
}
...
function isAngryDriverWalkable(x, y, entitySize = 20) {
  ...
  const radius = Math.max(0, entitySize * 0.45);
  ...
  return samples.every(([offsetX, offsetY]) => {
    ...
    // Explicitly block yellow buildings/objects
    const isYellow = (r > 150 && g > 140 && (r - b) > 50 && (g - b) > 50);
    if (isYellow) return false;
    ...
    // Surface allowances: Grey road/shadow, Green grass, and White/light surfaces
    const isGreyRoad = (Math.abs(r - g) < 45 && Math.abs(g - b) < 45 && Math.abs(r - b) < 45 && r > 20);
    const isGreenGrass = (g > r + 5 && g > b) || (g > r && g > b + 5);
    const isWhiteSurface = (r > 180 && g > 180 && b > 180);

    return isGreyRoad || isGreenGrass || isWhiteSurface;
  });
}
...
class NavigationSystem {
  ...
  for (let gy = 0; gy < this.gridHeight; gy++) {
    ...
    for (let gx = 0; gx < this.gridWidth; gx++) {
      ...
      if (typeof isAngryDriverWalkable === "function") {
        // Reserve clearance for the full AngryDriver body.
        walkable = isAngryDriverWalkable(world.x, world.y, 24);
      } else if (typeof isAICarWalkable === "function") {
        walkable = isAICarWalkable(world.x, world.y);
      }
