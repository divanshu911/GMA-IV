...
function isGrassOrRoad(x, y) {
    ...
    const isRoad = (Math.abs(r - g) < 30 && Math.abs(g - b) < 30 && r > 30);
...
function isRoadColor(x, y) {
  ...
  const isWhiteOrYellowLine = (r > 190 && g > 190 && b > 140); // Includes white and yellow road lines

  return isGreyRoad || isWhiteOrYellowLine;
}
...
function isPlayerCarWalkable(x, y) {
  ...
  const isBrightLine = (r > 190 && g > 190 && b > 140);
  const isGreen = (g > r + 15 && g > b + 15);

  return (isGreyWhiteOrShadow || isBrightLine || isGreen);
}
...
function isStrictRoadColor(x, y) {
  ...
  // Removed upper bound 'r < 220' so white road markings pass validation
