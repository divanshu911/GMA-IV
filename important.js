...
window.addEventListener('keydown', e => {
    ...
    if (e.key === 'e' || e.key === 'E') {
        ...
        if (playerCar) {
            triggerKeyboardButton('exitBtn', 'pointerdown');
        } else {
            triggerKeyboardButton('jackBtn', 'pointerdown');
        }
    }
    ...
    if (e.key === 'h' || e.key === 'H') {
        ...
        if (showFullMap) {
            ...
            x = rect.left + canvas.width - radarRadius - padding;
            y = rect.top + radarRadius + padding;
        }
    ...
    // F = Use whatever interact button is currently visible
    if (e.key === 'f' || e.key === 'F') {
        e.preventDefault();
        ...
        for (const id of interactButtonIds) {
            ...
            if (
                ...
                triggerKeyboardButton(id, 'click');
                break;
            }
        }
    }
    ...
    if (e.key === 'l' || e.key === 'L') {
        e.preventDefault();
        triggerKeyboardButton('towTruckBtn', 'click');
    }

    // G = Siren
    if (e.key === 'g' || e.key === 'G') {
        e.preventDefault();
        triggerKeyboardButton('sirenBtn', 'click');
    }
});
...
// Trigger an existing button listener without duplicating its gameplay logic
function triggerKeyboardButton(id, eventType = 'click') {
    const button = document.getElementById(id);
