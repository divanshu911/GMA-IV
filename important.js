...
if (isNaN(gameSeconds)) {
    gameSeconds = DAY_LENGTH * 0.25; // Default to 06:00
}
...
function updateDayNight(dt){
   ...
   if(hour < 5){
       darkness = 0.50; // Increased from 0.40 for a much darker night
       skyColor = "rgba(5, 10, 30, 0.6)"; // Deeper midnight blue
   } else if(hour < 7){
       let k=(hour-5)/2;
       darkness = 0.50 * (1 - k); // Smooth transition down to day
       ...
       let k=(hour-18)/2;
       darkness = 0.50 * k; // Smooth transition up
       skyColor = `rgba(${200 * (1-k) + 5*k}, ${120 * (1-k) + 10*k}, ${80 * (1-k) + 30*k}, ${0.6 * k})`;
   } else{
       darkness = 0.50; // Increased from 0.40
