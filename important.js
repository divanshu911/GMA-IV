...
// --- POLICE OFFICER CHASE WALKABILITY ---
// Slightly more tolerant than the normal AngryDriver collision check.
// This is only used while a police officer is chasing the player.
// It still blocks buildings, water, etc. but allows the officer to
// cross road/grass boundary areas more reliably.
function isPoliceOfficerWalkable(x, y, entitySize = 20) {
    if (!collisionData || mapWidth === 0) return true;

    // Keep the officer's body reasonably clear of solid terrain,
    // but don't require every outer sample to be walkable.
    const radius = Math.max(0, entitySize * 0.35);
    const diagonal = radius * 0.707;
    ...
    // Center MUST be walkable.
    // At least 6/9 samples must also be walkable.
    return walkableCount >= 6;
}
