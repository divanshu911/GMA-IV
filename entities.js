...
class NPC extends Pedestrian {
  constructor(id, x, y, shirt, hair, skin, isPolice = false) {
    // If it's a police officer, force dark navy shirt (#0b1d3a)
...
class Car {
  constructor(id, x, y, color, isPolice = false) {
    this.id = id;
    this.x = x;
    this.y = y;
    this.isPolice = isPolice;
    ...
    } else if (this.type === "Commuter, Sedan") {
      this.width = 16;
      this.length = 28;
  ...
  updateAI(dt, player, npcs, allCars) {
    ...
    if ((!movedOnX || !movedOnY) && this.speed > 0.5) {
      this.health = Math.max(0, this.health - this.speed * 1); 
      this.speed *= 0.3;
    }
