...
// --- 2. UPDATE NPC CLASS FOR CONVERSATIONS & FLEEING ---
...
class AngryDriver extends Pedestrian {
  constructor(x, y, targetCarColor, targetCarAngle, isPolice = false, targetCar = null) {
    // Police officers wear dark blue shirts (#0b1d3a), standard drivers wear car color
    ...
    this.reactionDelay = 110;
    this.isPolice = isPolice;
    this.targetCar = targetCar; // Store reference to original car
  }

  update(dt, targetEntity, onCatchPlayer) {
    if (this.reactionDelay > 0) {
      this.reactionDelay -= 1 * dt;
      return true;
    }

    // Always target the player (whether on foot or in a vehicle)
    let dx = targetEntity.x - this.x;
    let dy = targetEntity.y - this.y;
    let dist = Math.sqrt(dx * dx + dy * dy);

    if (dist > 330) return false; // Give up / despawn if player gets away

    if (dist < 30) {
      onCatchPlayer(this);
      return false; // Remove driver from active chasing list
    }

    let angleToTarget = Math.atan2(dy, dx);
    let nextNx = this.x + Math.cos(angleToTarget) * (this.speed * dt);
    let nextNy = this.y + Math.sin(angleToTarget) * (this.speed * dt);
  ...
  draw(ctx) {
    ...
    // Render speech bubble upright above AngryDriver
    this.drawSpeechBubble(ctx);
...
// === IMPROVED DYNAMIC CAR CLASS WITH POLICE VEHICLE SUPPORT ===
class Car {
  ...
  updateAI(dt, player, npcs, allCars) {
    ...
    // --- STOP DESTROYED / BLASTED CARS ENTIRELY ---
    ...
    if (distMoved < 0.25) this.stuckTimer += 1 * dt;
    else if (!this.isRecovering) this.stuckTimer = Math.max(0, this.stuckTimer - 0.5 * dt);

    if (this.stuckTimer > 90 && !this.isRecovering) {
      this.isRecovering = true;
      this.recoveryTimer = 100;
      let leftClear = this.checkRayWalkable(this.x, this.y, -0.6, 30);
      this.recoveryDir = leftClear ? -1 : 1;
    }
    ...
    if (this.isRecovering) {
      ...
      this.angle += (this.recoveryDir * 0.025) * dt;
    ...
    const scanRadius = 45, blindSpotAngle = 0.75;
    ...
    const processObstacle = (targetX, targetY, weight) => {
      ...
      if (dist < scanRadius) {
        ...
        steerFactor += (crossProduct > 0 ? -weight : weight);
      }
    };

    if (player) processObstacle(player.x, player.y, 0.05);
    if (npcs) npcs.forEach(npc => processObstacle(npc.x, npc.y, 0.03));
    if (allCars) {
      allCars.forEach(otherCar => {
        if (otherCar.id === this.id) return;
        let dx = otherCar.x - frontBumperX, dy = otherCar.y - frontBumperY;
        let dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < scanRadius + 10) {
          let angleToTarget = Math.atan2(dy, dx) + Math.PI / 2;
          let relativeAngle = angleToTarget - this.angle;
          while (relativeAngle < -Math.PI) relativeAngle += Math.PI * 2;
          while (relativeAngle > Math.PI) relativeAngle -= Math.PI * 2;
          ...
          if (Math.abs(relativeAngle) < blindSpotAngle + 0.2) {
            brakeRequired = true;
            if (dist < 30) emergencyBrake = true;
          }
          let crossProduct = Math.cos(this.angle) * dx - Math.sin(this.angle) * dy;
          steerFactor += crossProduct > 0 ? -0.04 : 0.04;
        }
      });
    }

    let hFactor = (this.health > 0) ? Math.max(0.75, this.health / this.maxHealth) : 0;

    // If exploded or dead health, freeze steering and stop the car
    if (this.health <= 0 || this.exploded) {
      this.speed = Math.max(0, this.speed - 0.15 * dt); // Rapidly come to a full stop
    } else {
      // Normal driving & obstacle speed adjustments
      if (emergencyBrake) {
        this.speed = Math.max(0, this.speed - 0.15 * dt);
      } else if (brakeRequired) {
        this.speed = Math.max(0, this.speed - 0.05 * dt);
      } else {
        this.speed += (0.04 * hFactor) * dt;
        if (this.speed > this.baseSpeed * hFactor) this.speed = this.baseSpeed * hFactor;
      }

      // --- 2. STEERING CONTROL (ONLY ACTIVE WHEN ALIVE) ---
      this.angle += steerFactor * dt;

      let isLeftClear = this.checkRayWalkable(frontBumperX, frontBumperY, -0.35, this.sensorLength);
      let isRightClear = this.checkRayWalkable(frontBumperX, frontBumperY, 0.35, this.sensorLength);
      let isCenterClear = this.checkRayWalkable(frontBumperX, frontBumperY, 0, this.sensorLength);
      ...
      if (this.speed > 0.05) {
        if (!isLeftClear && isRightClear) {
          ...
          let turnDir = (this.id % 2 === 0) ? 1 : -1;
          this.angle += (this.turnSpeed * 0.8 * turnDir) * dt;
        }
      }
    }
    ...
    if (isAICarWalkable(nextX, this.y)) { this.x = nextX; }
    if (isAICarWalkable(this.x, nextY)) { this.y = nextY; }
...
function initSpawns() {
  ...
  // 2. Spawn AI Cars (THIS WAS MISSING)
  for (let i = 0; i < NUM_CARS; i++) {
    let pos = getRandomRoadPosition();
    let isPolice = Math.random() < 0.15; // 15% chance for police vehicle
    let color = carColors[Math.floor(Math.random() * carColors.length)];

    cars.push(new Car(i, pos.x, pos.y, color, isPolice));
  }
}
