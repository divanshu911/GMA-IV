...
class AngryDriver extends Pedestrian {
  ...
  update(dt, targetEntity, onCatchPlayer) {
    ...
    if (dist > 330) { 
    // Driver gives up! If car was stolen, report it to police
    if (this.targetCar && this.targetCar.isStolen && !player.wanted) {
        player.wanted = true;
        localStorage.setItem("gma_player_wanted", "true");

        // Save stolen car details to local storage
        const stolenCarData = {
            id: this.targetCar.id,
            x: this.targetCar.x,
            y: this.targetCar.y,
            color: this.targetCar.color,
            type: this.targetCar.type,
            angle: this.targetCar.angle
        };
...
this.assignedOfficer = null;
  ...
  updateAI(dt, player, npcs, cars) {
    // --- FIX 1: DECOUPLE PLAYER & EMPTY CARS FROM AI CONTROL ---
    ...
    // --- FIX 2: DYNAMIC OBSTACLE DETECTION (PLAYER, NPCS, CARS) ---
    ...
    const checkObstacle = (obsX, obsY, obsRadius) => {
        ...
        if (dist > 0 && dist < detectDist + obsRadius) {
            ...
            // Check if obstacle is inside forward cone (+-40 degrees)
            if (Math.abs(diff) < 0.7) {
                obstacleInFront = true;
                if (diff < 0) avoidRight = true; // Obstacle to left -> steer right
                else avoidLeft = true;           // Obstacle to right -> steer left
            }
        }
    };

    // 1. Scan for Player on foot
    if (typeof player !== 'undefined' && player && (!playerCar || playerCar.id !== this.id)) {
        checkObstacle(player.x, player.y, 16);
    }

    // 2. Scan for Pedestrians (NPCs)
    if (npcs && npcs.length) {
        for (let i = 0; i < npcs.length; i++) {
            let npc = npcs[i];
            if (!npc.isPassenger) {
    ...
    // Inside Car.updateAI(dt, player, npcs, cars):
// Locate where Steering & Speed Control decisions are made, and add chaseTarget steering support:

if (this.chaseTarget && !obstacleInFront) {
  // Steer towards police chase target position
  let targetAngle = Math.atan2(this.chaseTarget.y - this.y, this.chaseTarget.x - this.x) + Math.PI / 2;
  let diff = targetAngle - this.angle;
  while (diff < -Math.PI) diff += Math.PI * 2;
  while (diff > Math.PI) diff -= Math.PI * 2;

  this.angle += Math.sign(diff) * Math.min(Math.abs(diff), this.turnSpeed * 1.5 * dt);
  
  // Accelerate during pursuit
  if (this.speed < this.baseSpeed * 1.6) {
    this.speed += 0.05 * dt;
  }
} else if (obstacleInFront) {
  // Existing obstacle avoidance logic...
  ...
  if (avoidRight && rightRoad) {
    ...
    if (obstacleInFront) {
        // Brake smoothly when blocked
        this.speed = Math.max(0, this.speed - 0.08 * dt);

        // Attempt steering around obstacle if road space permits
        if (avoidRight && rightRoad) {
            this.angle += 0.035 * dt;
        } else if (avoidLeft && leftRoad) {
            this.angle -= 0.035 * dt;
        ...
        // Steer away from off-road areas
    ...
    // Trigger reverse maneuver if stuck behind obstacle or wall too long (~0.85s)
...
function initSpawns() {
  ...
  // 2. Load Saved Stolen Car (if exists)
  let savedStolenCar = localStorage.getItem("stolen car");
  if (savedStolenCar) {
    try {
      let carData = JSON.parse(savedStolenCar);
      let stolenCar = new Car(carData.id, carData.x, carData.y, carData.color, false, carData.type);
      stolenCar.angle = carData.angle;
      stolenCar.isStolen = true;
      stolenCar.hasDriver = false; // Player or world owns it now
      cars.push(stolenCar);
    } catch (e) {
