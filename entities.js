...
class AngryDriver extends Pedestrian {
  constructor(x, y, targetCarColor, targetCarAngle, isPolice = false, targetCar = null) {
    ...
    // Player escaped out of range (driver gives up)
    if (dist > 330) { 
      return false; // Despawn driver and let updateGame trigger wanted state
    }

    // Player got caught
    if (dist < 30) {
      onCatchPlayer(this);
      return false; // Remove driver without wanted status
    }
