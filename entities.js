...
class AngryDriver extends Pedestrian {
  ...
  update(dt, targetEntity, onCatchPlayer, npcs = [], cars = []) {
    ...
    // Target current path waypoint or target entity directly
    ...
    let nextNx = this.x + Math.cos(moveAngle) * (this.speed * dt);
    let nextNy = this.y + Math.sin(moveAngle) * (this.speed * dt);

    // Verify valid color terrain (grey/green) and clear entity space
    let xValid = isAngryDriverWalkable(nextNx, this.y) && !this.isBlockedByEntity(nextNx, this.y, npcs, cars, targetEntity);
    let yValid = isAngryDriverWalkable(this.x, nextNy) && !this.isBlockedByEntity(this.x, nextNy, npcs, cars, targetEntity);

    if (xValid) this.x = nextNx;
    if (yValid) this.y = nextNy;

    // Local obstacle steering bypass if direct step is blocked
    if (!xValid && !yValid) {
      for (let offset of [0.6, -0.6, 1.2, -1.2]) {
        let altAngle = moveAngle + offset;
        let altX = this.x + Math.cos(altAngle) * (this.speed * dt);
        let altY = this.y + Math.sin(altAngle) * (this.speed * dt);
        if (isAngryDriverWalkable(altX, altY) && !this.isBlockedByEntity(altX, altY, npcs, cars, targetEntity)) {
          this.x = altX;
          this.y = altY;
          break;
        }
