...
class AngryDriver extends Pedestrian {
  ...
  update(dt, targetEntity, onCatchPlayer, npcs = [], cars = []) {
    ...
    if (!moved) {
      for (let offset of [0.55, -0.55, 1.05, -1.05, 1.57, -1.57, 2.2, -2.2]) {
    ...
    if (!moved) {
      // Force a fresh A* route after a blocked frame instead of repeatedly
      // pushing into the same building edge.
      this.path = [];
      this.pathIndex = 0;
      this.repathTimer = 0;
    }
    ...
    if (Math.hypot(actualDx, actualDy) > 0.001) {
      // Pedestrian art faces up by default, hence the +PI/2 correction.
