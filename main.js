    ...
    jackBtn.addEventListener('pointerdown', (e) => {
        ...
        if (targetCar && !playerCar) {
            ...
            if (targetCar.hasDriver) {
                targetCar.isStolen = true; // Mark vehicle as stolen
                ...
                // Sync wanted state & array in local storage
...
function updateGame(dt) {
  ...
  if (typeof updateDayNight === 'function') updateDayNight(dt);
    ...
    if (stolenCars.length > 0) {
        ...
        // If no stolen cars remain anywhere in world
        if (player.wanted) {
            player.wanted = false;
            updateStolenCarsStorage();
        }
...
cars.forEach(car => {
    if (car.isPolice && car.sirenAudio && car.sirenState !== 0) {
        ...
        const dx = player.x - car.x; //[span_3](start_span)[span_3](end_span)
        const dy = player.y - car.y; //[span_4](start_span)[span_4](end_span)
...
window.addEventListener('load', () => {
    ...
    if (savedSecondCarData) {
        ...
        // Restore stolen cars from array
    let savedStolenCars = localStorage.getItem("stolen_cars");
    if (savedStolenCars) {
        try {
            let stolenList = JSON.parse(savedStolenCars);
            if (Array.isArray(stolenList) && stolenList.length > 0) {
                stolenList.forEach((sData) => {
                    let restoredCar = new Car(sData.id || (Date.now() + Math.random()), sData.x, sData.y, sData.color, false, sData.type);
                    if (typeof applyCarStats === 'function') applyCarStats(restoredCar, sData.type);
                    restoredCar.angle = sData.angle || 0;
                    restoredCar.isParked = true;
                    restoredCar.hasDriver = false;
                    restoredCar.isStolen = true;
                    cars.push(restoredCar);
                });
                player.wanted = true;
            }
        } catch(e) {
            console.error("Failed to parse stolen_cars from storage:", e);
        }
    }
