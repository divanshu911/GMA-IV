...
function updateGame(dt) {
  ...
  if (typeof updateDayNight === 'function') updateDayNight(dt);
    ...
    if (player.wanted) {
        let stolenCar = cars.find(c => c.isStolen);

        if (stolenCar) {
            // Keep updating saved position if player is driving it
            ...
            // Check distance if player abandoned the stolen car (outside vehicle)
            if (!playerCar || playerCar.id !== stolenCar.id) {
                ...
                if (dist >= 800) {
                    ...
                    // HUD Notification
                    if (typeof taxiManager !== 'undefined' && taxiManager.setMessage) {
                        taxiManager.setMessage("Stolen car was found by police!", 180);
                    }
                }
            }
        }
