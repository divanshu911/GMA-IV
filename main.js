console.log("fixed repair system");
// --- 1. AUDIO & STATE ---
...
// Plays sound from pool only if event is within maxDistance, attenuating volume based on proximity
...
function handlePhysicsAndCollisions() {
  for (let i = 0; i < cars.length; i++) {
    for (let j = i + 1; j < cars.length; j++) {
      ...
      if (dist < 26) {
        ...
        if (relSpeed > 0.6) { // Small bumps ignored
        // --- NEW: CAR CRASH AUDIO (Spatial / Proximity Based) ---
            ...
            // ONLY apply damage if the player is actively driving one of the involved vehicles
            ...
            let tryExplode = (car, strikingCar, speed) => {
                if (speed > 2.8 && strikingCar.weightMultiplier >= 2.5 && Math.random() < (speed * 0.12) && !car.exploded) {
                    ...
                    if (typeof playerCar !== 'undefined' && playerCar && car.id === playerCar.id) {
                        player.health -= 60; // Hurt player heavily
                        ...
                        checkPlayerDeath(); // Check if they survived the blast
                    }
                }
            };
  ...
  cars.forEach(car => {
    npcs.forEach(npc => {
      ...
      if (dist < 22) {
          // --- NEW: CAR HITS NPC AUDIO (Spatial / Proximity Based) ---
  ...
  if (!isInsideHouse && !isInsideDealership) {
      ...
      // --- NEW: NPC to NPC COLLISION AUDIO (Spatial / Proximity Based) ---
...
function updateGame(dt) {
  ...
  if (!isInsideHouse && !isInsideDealership) {
      ...
      // --- NEW: VEHICLE AUDIO STATES (Engine Dead & Fire Loop with Spatial Checks) ---
      cars.forEach(car => {
  ...
  // REPAIR GARAGE & TOW TRUCK BUTTON VISIBILITY
  if (!isInsideHouse && !isInsideDealership) {
  ...
  if (!playerCar) {
    ...
    if (isInsideDealership) {
      ...
      // Hide jack button if vehicle is exploded
  ...
  if (playerCar) {
    if (isMoving) {
      ...
      while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
        ...
        if (playerCar.health <= 0) playerCar.speed = 0; // Engine stops completely
    ...
    if (typeof isPlayerCarWalkable === 'function') {
        ...
        if (hitWall && playerCar.speed > 0.7) {
            ...
            playerCar.health = Math.max(0, playerCar.health - playerCar.speed * 3); // Heavy wall damage
            playerCar.speed *= 0.4; // Lose momentum
        }
    } else {
  ...
  if (typeof showFullMap !== 'undefined' && showFullMap) {
    ...
    if (mapImage.complete && mapWidth > 0) {
      ...
      ctx.fill(); ctx.stroke();
          ...
          // Repair Garage Marker on Full Map
