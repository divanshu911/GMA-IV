...
collisionMapImage.src = "https://raw.githubusercontent.com/divanshu911/My-game-assets/refs/heads/main/CollisionMap.png";
    ...
    for (let j = i + 1; j < cars.length; j++) {
      ...
      if (dist < 26) {
        ...
        let relSpeed = Math.sqrt(Math.pow(vx1 - vx2, 2) + Math.pow(vy1 - vy2, 2));
          ...
          // Ensure collision audio only plays if cars are actually moving relative to each other 
          // AND at least one car has active movement speed
          const isCarMoving = Math.abs(c1.speed || 0) > 0.2 || Math.abs(c2.speed || 0) > 0.2;

          if (relSpeed > 0.6 && isCarMoving && !c1.exploded && !c2.exploded && c1.health > 0 && c2.health > 0) {
