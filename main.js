...
function buildCollisionGrid(entities) {
    collisionGrid.clear();

    for (const entity of entities) {
        if (!entity) continue;

        const key = getCollisionCell(entity.x, entity.y);
...
function getNearbyEntities(entity) {
    ...
    for (let ox = -1; ox <= 1; ox++) {
        for (let oy = -1; oy <= 1; oy++) {
            const cell = collisionGrid.get(`${cellX + ox},${cellY + oy}`);
