...
mapImage.onload = () => {
    ...
    console.log("Map loaded:", mapWidth, "x", mapHeight);
};
...
document.getElementById('blackMarketBtn').addEventListener('click', () => {
  ...
  if (carToSell.ownerType !== "civilian") {
    ...
    let carType = carToSell.type || "Commuter, Sedan"; // Default to sedan if type isn't explicitly set
    let price = 50; 

    if (carType === "Hauler, Truck") {
        price = 70;
    } else if (carType === "Falcon, Sports") 
{price = 95;}
   else if (carType === "Ranger, SUV") {price = 80;}
    else if (carType === "Porter, Van") {price = 65;} 
    else if (carType === "Sprint, Hatchback") 
{price = 40;}
...
// 1. Auto-save player coordinates every 3 seconds
setInterval(() => {
    // Ensure player object exists and player is not in the house
    if (typeof player !== 'undefined' && !isInsideHouse) {
        localStorage.setItem("gma_player_x", player.x.toFixed(2));
        localStorage.setItem("gma_player_y", player.y.toFixed(2));
    }
}, 3000); 

// 2. Movement Tracker for Respawn Eligibility
let lastRespawnCheckX = 0;
let lastRespawnCheckY = 0;
let recentMovementDistance = 0;

// Updates the player's movement history every 2 seconds
setInterval(() => {
    if (typeof player !== 'undefined') {
...
respawnBtn.style.display = 'none'; // Hidden by default
document.getElementById('gameContainer').appendChild(respawnBtn);

// 4. Click Event & HUD Messages
respawnBtn.addEventListener('click', () => {
    ...
    if (status.active) {
        ...
        // Close the map and resume the game loop
        if (typeof showFullMap !== 'undefined') {
            showFullMap = false;
            gameActive = true;
        }
    } else {
        // Display HUD message explaining why the button is inactive
        taxiManager.setMessage(status.reason, 240);
    }
});

// 5. Standalone UI Updater for the Button
function updateRespawnButtonUI() {
    // Only display when the full world map view is active
    if (typeof showFullMap !== 'undefined' && showFullMap) {
        respawnBtn.style.display = 'block';

        let status = getRespawnStatus();

        // Toggle Colors Based on Activity Status
        if (status.active) {
            respawnBtn.style.backgroundColor = '#f1c40f'; // Active Yellow
            respawnBtn.style.color = '#000000';           // Black text
        } else {
            respawnBtn.style.backgroundColor = '#7f8c8d'; // Inactive Grey
            respawnBtn.style.color = '#ffffff';           // White text
        }
    } else {
        respawnBtn.style.display = 'none';
    }

    // Loop continuously alongside the main game
    requestAnimationFrame(updateRespawnButtonUI);
}

// Initialize the button UI tracker
