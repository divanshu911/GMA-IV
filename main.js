...
class TruckJobManager {
  ...
  update(dt, player, cars) {
    ...
    if (playerCar !== this.truck && this.truck) {
      ...
      if (distToTruck > 300) { 
        ...
        taxiManager.setMessage("Get back in the truck! Don't leave it to avoid penalty.", 2);
      }
    }
    ...
    if (playerCar === this.truck) {
      if (this.stage === 1) {
        ...
        if (dist < 60) {
          ...
          taxiManager.setMessage("Goods Loaded! Deliver them safely to the green waypoint.", 240);
        }
        ...
        if (dist < 60) {
          ...
          this.destinationX = 0;
          this.destinationY = 0;
          taxiManager.setMessage("Goods Delivered! Return the truck to the Truck Company zone.", 240);
        }
        ...
        if (dist < 60) {
          ...
          player.money += reward;
          localStorage.setItem("gma_player_money", player.money);
          taxiManager.setMessage(`Cargo Completed! Job Pay: +$${reward}`, 240);
          this.cleanup(cars);
        }
      }
    }
  ...
  drawWorldMarkers(ctx) {
    // All truck zones are intentionally invisible in the world view.
...
truckBtn.innerHTML = '🚛';
truckBtn.style.right = '70%'; // Placed cleanly next to the home/restaurant buttons
...
blackMarketBtn.style.right = '50%'; // Cleanly placed next to the other buttons
